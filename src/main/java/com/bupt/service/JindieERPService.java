...
import com.bupt.utils.InvokeHelper;
...
public class JindieERPService {
    ...
    public List<JindieERPPurchaseOrder> importPUR() throws Exception {
        ...
        if (InvokeHelper.Login(dbId, uid, pwd, lang)) {
            ...
            Date back = DateUtils.addDays(current,-2);
        ...
        for(JindieERPPurchaseOrder jindieERPPurchaseOrder:jindieERPPurchaseOrders){
            if(!heads.containsKey(jindieERPPurchaseOrder.getFBillNo())){
                ...
                headAndDetail.getHead().setWarehouseId(1);
        ...
        for(int i = 0;i<headAndDetailList.size();i++){
            ...
            if(purchaseOrder==null){
                ...
                purchaseOrderDAO.insert(headAndDetail.getHead());
                ...
                headAndDetailList.get(i).getHead().setId(purchaseOrder.getId());
                ...
                for(int j = 0;j<headAndDetailList.get(i).getDetails().size();j++){
                    ...
                    System.out.println(headAndDetailList.get(i).getDetails().get(j));
        ...
        return jindieERPPurchaseOrders;
    }
}
